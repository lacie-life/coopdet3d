...
class GlobalRotScaleTransCoop:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        #    visualize_feature_lidar(data["vehicle_points"].tensor.numpy(), "/home/bevfusion/viz_tumtraf_featmap/features/lidar/vehicle/val_globalrotscaletrans")
...
class VehiclePointsToInfraCoords:
    def __call__(self, data):
        ...
        # Option 2: TODO: transform points
        # print shape of v2i
        # print(v2i.shape)
        # make sure v2i is a 4x4 matrix
        # assert v2i.shape == (4, 4)
        # # make vehicle_point homogeneous x,y,z,1
        # vehicle_points = data["vehicle_points"].tensor
        # vehicle_points = torch.cat([vehicle_points, torch.ones_like(vehicle_points[:, :1])], dim=1)
        # # transform vehicle_points to infrastructure coordinate system
        # # TODO: check whether it needs to be inversed
        # vehicle_points_in_infra_cs = torch.matmul(vehicle_points, torch.from_numpy(v2i).float())
        # # remove homogeneous dimension
        # vehicle_points_in_infra_cs = vehicle_points_in_infra_cs[:, :3]
        # # update vehicle_points
        # data["vehicle_points"].tensor = vehicle_points_in_infra_cs

        # if not self.is_train:
        #    visualize_feature_lidar(data["vehicle_points"].tensor.numpy(), "/home/bevfusion/viz_tumtraf_featmap/features/lidar/vehicle/pipelinev2i")
        #    visualize_feature_lidar(data["infrastructure_points"].tensor.numpy(), "/home/bevfusion/viz_tumtraf_featmap/features/lidar/vehicle/pipelineinfra")
...
# STILL BROKEN, EXCLUDE FOR NOW
@PIPELINES.register_module()
class RandomFlip3DCoop:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        flip_horizontal_infrastructure = random.choice([0, 1])
        flip_vertical_infrastructure = random.choice([0, 1])

        rotation_vehicle = np.eye(3)
        rotation_infrastructure = np.eye(3)

        if flip_horizontal_infrastructure:
            rotation_infrastructure = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]) @ rotation_infrastructure
            rotation_vehicle = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]) @ rotation_vehicle
            if "infrastructure_points" in data:
                data["infrastructure_points"].flip("horizontal")
            if "vehicle_points" in data:
                data["vehicle_points"].flip("horizontal")
            if "gt_bboxes_3d" in data:
                data["gt_bboxes_3d"].flip("horizontal")
            if "gt_masks_bev" in data:
                data["gt_masks_bev"] = data["gt_masks_bev"][:, :, ::-1].copy()

        if flip_vertical_infrastructure:
            rotation_infrastructure = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]) @ rotation_infrastructure
            rotation_vehicle = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]) @ rotation_vehicle
            if "infrastructure_points" in data:
                data["infrastructure_points"].flip("vertical")
            if "vehicle_points" in data:
                data["vehicle_points"].flip("vertical")
            if "gt_bboxes_3d" in data:
                data["gt_bboxes_3d"].flip("vertical")
            if "gt_masks_bev" in data:
                data["gt_masks_bev"] = data["gt_masks_bev"][:, ::-1, :].copy()

        data["vehicle_lidar_aug_matrix"][:3, :] = rotation_vehicle @ data["vehicle_lidar_aug_matrix"][:3, :]
        data["infrastructure_lidar_aug_matrix"][:3, :] = rotation_infrastructure @ data[
...
class ObjectPasteCoop:
    ...
    def __call__(self, data):
        ...
        # change to float for blending operation
        vehicle_points = data["vehicle_points"]  # In infra coord
        infrastructure_points = data["infrastructure_points"]  # In infra coord

        if self.sample_2d:  # Ignore for now since sample_2d is always false
            img = data["img"]
            gt_bboxes_2d = data["gt_bboxes"]
            # Assume for now 3D & 2D bboxes are the same
            sampled_dict = self.db_sampler.sample_all(
